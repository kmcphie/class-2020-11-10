...
```{r}
tibble(true = week_10$reg_chg,
       fitted = predict(fit_obj)) %>%
  mutate(residual = true - fitted,
         squared = residual^2) %>%
  summarize(rmse = sqrt(mean(squared)))
```


* Write a sentence or two describing a situation in which RMSE would not be a good metric for choosing among models.

If there are a lot of outliers, using RMSE and squaring things would control the model whereas MAD would be a lot more accurate.
...
```{r}
set.seed(30)
week_10_split <- initial_split(week_10, prob = 0.80)
